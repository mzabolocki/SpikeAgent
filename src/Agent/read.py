...
def stack_recordings(pack_folder_pre, trigger = True):
    cont_data_all = []
    cont_trigger_all = []
            ...
            if '.rhd' in i:
                ...
                if sampling_freq > 20000:
                    ...
                    for j in tqdm(range(data.shape[1])):
                        new_data_j = signal.resample(data[:,j], int(data.shape[0] / resample_factor))
    ...
    recording = se.NumpyRecording(traces_list=cont_data_all, sampling_frequency=20000)
    
    return recording
    

def read_data_folder(params, folder_paths):
    is_neuropixel = params['is_neuropixel']
    output_folder = folder_paths.output_folder
    raw_data_folder = folder_paths.raw_data_folder
    reocrding_folder = folder_paths.recording_folder
    if not output_folder.exists(): output_folder.mkdir()
    if not reocrding_folder.exists():
        if is_neuropixel:
            stream_names, stream_ids = si.get_neo_streams('spikeglx', raw_data_folder)
            if len(stream_names) > 1:
                for i, name in enumerate(stream_names):
                    print(f'{i} {name}')
                idx = input("Enter number of target stream format")
                target = stream_names[idx]
            else:
                target = stream_names[0]
            print(target)
            recording = si.read_spikeglx(raw_data_folder, stream_name=target, load_sync_channel=False)
        else:
            recording1 = stack_recordings(raw_data_folder)
            recording_trace = recording1.get_traces()
            sampling_freq = recording1.get_sampling_frequency()
            recording = se.NumpyRecording(traces_list=recording_trace, sampling_frequency=sampling_freq)

        recording.save(folder=reocrding_folder)
    recording = sc.load_extractor(reocrding_folder)
    day_length = recording.get_total_duration()

    print(recording)
    print('Num. channels = {}'.format(len(recording.get_channel_ids())))
    print('Sampling frequency = {} Hz'.format(recording.get_sampling_frequency()))
    print('Num. timepoints seg0= {}'.format(recording.get_num_segments()))
    print(f"Recordings saved in {reocrding_folder}")
    
    return recording, day_length