...
@tool
def environment_setup_tool( is_neuropixel:bool, 
    ...
    Every code should be executed sequentially, except for updating params, which is conditional
    This tool:
    - If user wants default, use default.
    ...
    # Update params only when the user specifies parameters other than the default
    if raw_data_path is not None: params['raw_data_path'] = {raw_data_path}
    if processed_path is not None: params['processed_path'] = {processed_path}
    if folder_name is not None: params['folder_name'] = {folder_name}
...
@tool
def read_recording_tool(filter_frequency_minimum:float|None, 
                        filter_frequency_maximum:float|None, 
                        filter_operator:str|None, 
                        figure_height:float|None, 
                        probe_zoomin:bool|None
                        ) -> str:
    """
    ...
    - Shows the structure of the probe and compares the recording before and after filtering & common referencing.
    ...
    global recording
    global day_length
    global recording_cmr
    
    # Update params only when the user specifies parameters other than the default
    if filter_frequency_minimum is not None: params['filter_frequency_minimum'] = {filter_frequency_minimum}
    if filter_frequency_maximum is not None: params['filter_frequency_maximum'] = {filter_frequency_maximum}
    if filter_operator is not None: params['filter_operator'] = {filter_operator}
    if figure_height is not None: params['figure_height'] = {figure_height}
    if probe_zoomin is not None: params['probe_zoomin'] = {probe_zoomin}
    
    # Read recording
    recording, day_length = ar.read_data_folder(params, folder_paths)
    
    # Creat probe and plot the probe
    mesh_probe = au.create_probe(recording, params)
    av.show_probe(recording, params)
    
    # Filtering and common referencing, plot recordings of befor and after processing
...
@tool
def run_sorting_tool(   sorter_name:str|None,
                        load_if_exist:bool|None,
                        modify_sorting_param:dict|None
                    ) -> str:
    """
    ...
    Parameters:
    - sorter_name: name of sorter for the sorting. 
    - load_if_exist: Whether to load the saved sorting, or rerun the sorting.
    - modify_sorting_param: dictionary to update sorting params. Empty dictionary if user wants default.
    
    Tips:
    - If user wants what kind of sorters are avaliable, her are the lists: 'mountainsort4', 'mountainsort5', 'kilosort4', 'spykingcircus2', 'tridesclous2', 'tridesclous', 'herdingspikes'
    - if user wants to see what kind of sorting params are avaliale or to see default sorting params for this sorter,
    ...
    """
    code = f"""
    global sorting_params
    global sorting
    global we
    
    # This step is before curation
    params['curated'] = False
    
    # Update params only when the user specifies parameters other than the default
    if sorter_name is not None: params['sorter_name'] = {sorter_name}
    if load_if_exist is not None: params['load_if_exist'] = {load_if_exist}
    
    # Update folder path according to chooen sorter. It creates path for the sorting results
...
@tool
def initial_visualization_tool( save_figures:bool|None,
                                colored_template:bool|None,
                                n_columns:int|None,
                                figure_height:float|None
                            ) -> str:
    """
    ...
    # Update params only when the user specifies parameters other than the default
    if save_figures is not None: params['save_figures'] = {save_figures}
    if colored_template is not None: params['colored_template'] = {colored_template}
    if n_columns is not None: params['n_columns'] = {n_columns}
    if figure_height is not None: params['figure_height'] = {figure_height}
...
@tool
def curation_merging_tool(  min_snr:float|None,
                            max_isi:float|None,
    ...
    Parameters
    - max_isi: Maximum threshold for ISI(Inter-Spike-Interval) violation rate(Pecantage). Values between 0.5% and 1.5% are recommended.
    - min_snr: Minimum threshold for SNR(Signal-to-noise ratio). Values between 4.0 and 10.0 are recommended.
    - show_metrics: Determines whether to show the matrix or not.
    - load_if_exists: Whether to load the saved curation result, or rerun the curation.

    After execution, `keep_unit_ids`, `sorting_curated`, and `we_curated` will be available.
    """
    code = f"""
    global keep_unit_ids
    global metrics
    global sorting_curated
    global we_curated
    
    # This step is after curation
    params['curated'] = True
    
    # Update params only when the user specifies parameters other than the default
    if min_snr is not None: params['min_snr'] = {min_snr}
    if max_isi is not None: params['max_isi'] = {max_isi}
    if load_if_exist is not None: params['load_if_exist'] = {load_if_exist}
    if show_metrics is not None: params['show_metrics'] = {show_metrics}
    
    # Get good units by auto curation
    keep_unit_ids, metrics = au.auto_curation(we, params)
    
    # Plot overall data of units based on metrics
    av.summary(metrics,we,keep_unit_ids,params,folder_paths)
    
    # Get curated sorting and waveform based on curated units
...
@tool
def final_visualization_tool( save_figures:bool|None,
                                colored_template:bool|None,
                                n_columns:int|None,
                                figure_height:float|None
                            ) -> str:
    ...
    Every code should be executed sequentially, except for updating params, which is conditional
    This tool:
    - Shows rasters, templates, unit waveforms, and ISI rate of curated units
    - Users can choose whether to save, color the graph, and set the number of columns for each plot.
    ...
    # Plot sorting results of curated units
    ...
    # Save extremum channels of curated units
...
claude_3_5_sonnet_20241022 = ChatAnthropic(model_name="claude-3-5-sonnet-20241022",temperature=0).bind_tools(tools)
claude_3_5_sonnet_20240620 = ChatAnthropic_H(model_name="claude-3-5-sonnet-20240620-v1",temperature=0).bind_tools(tools)
claude_3_5_haiku_20241022 = ChatAnthropic(model_name="claude-3-5-haiku-20241022",temperature=0).bind_tools(tools)
claude_3_opus_20240229 = ChatAnthropic_H(model_name="claude-3-opus-20240229-v1",temperature=0).bind_tools(tools)
claude_3_haiku_20240307 = ChatAnthropic_H(model_name="claude-3-haiku-20240307-v1",temperature=0).bind_tools(tools)
claude_3_sonnet_20240229 = ChatAnthropic_H(model_name="claude-3-sonnet-20240229-v1",temperature=0).bind_tools(tools)
    ...
    if isinstance(last_message, ToolMessage) and hasattr(last_message, "artifact") and last_message.artifact and model_name != "claude_3_5_haiku":
        ...
        content_list = [{
            ...
            "text": """
                ...
                • Proceed directly to the next step without asking for user feedback, unless user ask for feedback in his initial input.
